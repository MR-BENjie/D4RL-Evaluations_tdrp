...
class Logger(object):
    ...
    def save_final_obs(self, itr, path):
        if self._snapshot_dir:
            if self._snapshot_mode == 'all':
                file_name = osp.join(self._snapshot_dir, 'path_itr_%d.pkl' % itr)
                torch.save(path, file_name)
            elif self._snapshot_mode == 'last':
                # override previous params
                file_name = osp.join(self._snapshot_dir, 'path.pkl')
                torch.save(path, file_name)
